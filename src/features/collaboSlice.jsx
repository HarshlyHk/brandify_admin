...
export const createCollabo = createAsyncThunk(
  ...
  async (collaboData, { rejectWithValue }) => {
    try {
      const { data } = await axiosInstance.post("/collabos/admin/add", collaboData);
...
export const updateCollabo = createAsyncThunk(
  ...
  async ({ id, collaboData }, { rejectWithValue }) => {
    try {
      const { data } = await axiosInstance.put(`/collabos/admin/update${id}`, collaboData);
...
export const toggleCollaboStatus = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await axiosInstance.patch(`/collabos/${id}/toggle-status`);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateCollabo.fulfilled, (state, action) => {
        const index = state.collabos.findIndex((c) => c._id === action.payload._id);
